...
## 1. 根幹アーキテクチャ・基本設計 (Core Architecture & Fundamental Design)

今後のコード変更・機能追加時において、AIや開発者が**絶対に破ってはならない**システムの基本設計および実装ルール（Invariants / Architecture）を定義する。
...
### 1.2 モジュール責務の境界定義 (責務分離の原則)
本プラグインはMVC/MVVM的な責務分離を採用しており、ファイルの配置ディレクトリ自体がそのコードの責務を表している。機能追加時はこの境界を越えた依存関係（スパゲッティ化）を作ってはならない。
* **`ui/`**: 画面表示とユーザー操作のオーケストレーション。状態を保持・管理し、ユーザー入力のバリデーションを行う。
* **`layer/`**: GeoPackage等の永続化、セッションファイル操作、およびシンボロジ（見た目）の状態管理。
* **`logic/`**: Qt・QGISキャンバスに依存しない、純粋な座標変換や重複判定などのドメインロジック。
* **`canvas/`**: QGISメインキャンバス上でのマウスイベント（クリック・ホバー）の受付のみを行う。

### 1.3 UI構築の絶対ルールとOSネイティブ保護
* **フレキシブルレイアウトの強制**: `.ui` ファイルを用いた静的レイアウト設計は禁止する。必ず `QHBoxLayout` などの比率（Stretch）を用いたPythonベースの宣言的UIで構築し、画面幅の変動に対応させること。
* **QSpinBox に対する QSS ハックの禁止**: OS標準のUI（特に上下矢印）を破壊してバグを誘発しないよう、透明ボーダーハックや base64 URI 指定を禁止する。状態変化は `src/icon/` 配下の絶対パスSVGの差し替えによって表現するという安全規約を厳守すること。

### 1.4 単一方向のシグナル・イベント駆動設計 (Unidirectional Data Flow)
コンポーネント間の双方向参照を断ち切る設計を維持する。
* **マップツール ➔ UI**: キャンバスツールはUIの状態を直接参照せず、クリック座標などの「シグナル」を投げるのみ。
* **UI ➔ マップツール**: フォーカスモードの条件などは、UI側から一方通行でプッシュ（反映）する。

### 1.5 サイレントエラーブロックの原則
入力エラー（点名重複や遺構名未指定など）が発生した際、確認モーダルダイアログ等を出してユーザーの手（ワークフロー）を止めてはならない。
**「UIの枠を赤くしたうえで、キャンバスクリックを静かに無視する（打刻させない）」** という、連続打刻業務に特化したフェイルセーフ原則を維持すること。

### 1.6 ダイアログのモードレス性と排他制御
画像管理・設定・出力ダイアログは、メインキャンバスを並行確認できるようすべて「モードレス」とする。
ただし、**いずれかのダイアログが開いている間は、意図しないデータ不整合を防ぐため、メインキャンバスの打刻ツールを強制サスペンドする**という状態安全のトレードオフ原則を守ること。
...
**ホバー強調およびスナップ判定（空間インデックス検索）自体を物理的にスキップし**、クリックした座標にそのまま新しい点を作成します。誤操作を完全に遮断する設計です。さらに内部で2つのサブモードに分岐します。
...
1. **空振りクリック時のモード維持**: 既存フィーチャの無い空白をクリックした場合、新規フィーチャは作成されず、**モードは『編集』のまま維持されます**。意図しない新規モードへの遷移や誤打刻を防ぐ安全装置です。
2. **データロード時の安全装置 (`_suppress_realtime_commit`)**: DBからUIパネルに値を展開する際、UI変更イベントが発火してDBへ書き戻してしまう無限ループを防ぐため、一時的にコミットを遮断します。
3. **リアルタイムコミット (T-0038)**: ロード完了後、ユーザーが点名や属性を変更すると、確定ボタンなしで直接DBへ書き込まれます。
4. **即時削除処理**: ユーザーが削除を選択した場合、確認ダイアログを挟まず、即座にDBトランザクション経由でフィーチャを破棄します。
...
2. **【対象図面変更時の空間的整合性制約】**: 一度打刻された点の所属図面（`drawing_name`）は変更可能とするが、変更先の画像の実ピクセル範囲内に物理座標が収まっているかを必ず検証し、収まっていない場合や未変換図面への変更時はDBの更新をブロックしなければならない。
...
1. **ワールドファイル拒否判定**: 「基準点設置」ボタン押下時 (`_on_confirm_image_clicked`)、ソース画像と同じ階層にワールドファイル (`.tfw`, `.jgw` 等) が存在しないか検査し、存在すれば本プラグインの管轄外として処理を拒否します。**ただし、この制約は「新規追加モード」のみが対象であり、既存レイヤの基準点を再編集する際には適用されません。**
...
3. **CSV書き出し**:
   ...
   * データは**「出土形態, 遺構名, 属性, 点名, 枝番, Ｘ座標(南北), Ｙ座標(東西)」の 7 項目のみにスリム化されてエクスポート**されます。不要なシステムカラムは出力しません（スキーマの固定化）。
